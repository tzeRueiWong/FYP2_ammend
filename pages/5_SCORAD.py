...
if st.session_state.user == '':
    ...
    with tab_1:
        ...
        if latest_date != None and is_same_week(latest_date, datetime.date.today()):
            ...
            if latest_score < 25:
                color = "green"
                condition = "Your condition is considered :green[mild]"
            elif latest_score >=25 and latest_score <= 50:
                color = "orange"
                condition = "Your condition is considered :orange[moderate]"
            else:
                color = "red" 
                condition = "Your condition is considered :red[severe]"

            st.subheader("Last record this week is on " + str(latest_date))
            st.subheader("Score: :" + color + "[" + str(latest_score) + "] out of 103")
